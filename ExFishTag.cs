namespace ExBuddy.OrderBotTags.Fish
{
	...
	public class ExFishTag : ExProfileBehavior
	{
		...
		protected override Composite CreateBehavior()
		{
			...
			return new PrioritySelector(
				...
				GoFish(
					...
					new ExCoroutineAction(ctx => HandleThaliaksFavor(), this),
					PatienceComposite,
					FishEyesComposite,
					ChumComposite,
					CastComposite,
		...
		private async Task<bool> HandleThaliaksFavor()
		{
			if (ThaliaksFavor == ThaliaksFavor.None)
			{
				// Not using cordials, skip method.
				return false;
			}
			...
			if (ThaliaksFavor == null)
			{
				ThaliaksFavor = ThaliaksFavor.None;
				return false;
			}

			if (!ActionManager.CanCast(26804, Core.Me))
			{
				// Unable to cast Thaliak's Favor
				return false;
			}
		...
		[DefaultValue(ThaliaksFavor.None)]
		[XmlAttribute("ThaliaksFavor")]
		public ThaliaksFavor ThaliaksFavor { get; set; }
